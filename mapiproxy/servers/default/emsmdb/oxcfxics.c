...
static bool oxcfxics_push_messageChange(struct emsmdbp_context *emsmdbp_ctx, struct emsmdbp_object_synccontext *synccontext, const char *owner, struct oxcfxics_sync_data *sync_data, struct emsmdbp_object *folder_object)
{
	...
	if (sync_data->message_sync_data) {
		...
		table_object->object.table->prop_count = 2;
		table_object->object.table->properties = talloc_zero_array(mem_ctx,
                                                                           enum MAPITAGS,
                                                                           table_object->object.table->prop_count);
                if (!table_object->object.table->properties) {
                        OC_DEBUG(1, "Error allocating table property list");
                        goto end;
                }
                table_object->object.table->properties[0] = PidTagMid;
                table_object->object.table->properties[1] = PidTagChangeNumber;
